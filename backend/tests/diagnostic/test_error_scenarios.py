...
class TestNotFoundErrors:
    ...
    def test_course_not_found_empty_catalog(self, mock_config):
        ...
        try:
            ...
            assert "No course found" in results.error, f"Expected 'No course found' in error, got: {results.error}"
    ...
    def test_course_not_found_via_search_tool(self, temp_chroma_db):
        ...
        result = tool.execute(
            query="test query",
            course_name="This Course Does Not Exist"
        )

        # Should return error message, not raise exception
        assert isinstance(result, str), f"Expected string result, got {type(result)}"
        assert "not found" in result.lower() or "No course found" in result, \
    ...
    def test_empty_search_results(self, temp_chroma_db):
        ...
        assert isinstance(result, str), f"Expected string result, got {type(result)}"
        assert "No relevant content found" in result, f"Expected 'No relevant content found', got: {result}"
...
class TestQueryFailedErrors:
    ...
    def test_chromadb_exception_handling(self, mock_chroma_client, mock_config):
        ...
        assert "Search error" in results.error, f"Expected 'Search error', got: {results.error}"
    ...
    def test_provider_api_error_propagation(self, mock_groq_class, mock_config):
        ...
        assert "rate limit" in str(exc_info.value).lower(), f"Expected 'rate limit' in error, got: {exc_info.value}"
