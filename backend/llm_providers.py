...
class GroqProvider(BaseLLMProvider):
    ...
    def __init__(self, api_key: str, model: str):
        debug_print(f"[DEBUG] Initializing GroqProvider with API key: {api_key[:20] if api_key else 'NONE'}...")
        debug_print(f"[DEBUG] Model: {model}")
        # Store API key instead of client for async-safe usage
        self.api_key = api_key
        self.model = model
        debug_print(f"[DEBUG] GroqProvider initialized")
    ...
    def generate_response(
        ...
        debug_print(f"[DEBUG] GroqProvider.generate_response called")
        debug_print(f"[DEBUG] self.api_key type: {type(self.api_key)}")
        debug_print(f"[DEBUG] self.api_key value: {self.api_key[:30] if self.api_key else 'NONE'}...")
        debug_print(f"[DEBUG] self.api_key length: {len(self.api_key) if self.api_key else 0}")

        # Create fresh client for each request (async-safe)
        from groq import Groq
        debug_print(f"[DEBUG] About to create Groq client...")
        ...
        debug_print(f"[DEBUG] Making Groq API call with params: {list(api_params.keys())}")

        # Get response from Groq
        try:
            response = client.chat.completions.create(**api_params)
            debug_print(f"[DEBUG] Groq API call successful")
